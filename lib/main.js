 ...
 */
function copy() {
  // TODO: decide on a standard format for designating blocks of code. files.js uses something completely different
  /*****************************
  ...
  log = logValues(files.getSpreadsheet());
  ...
  /*****************************
     ...
     */
  // TODO: could this be refactored so that this is just done once inside the loop?
  ...
  process.cleanup(properties, timer, triggers, files);
}
...
function processFileList(files, timer) {
  ...
  while (files.fileListExists() && timer.canContinue()) {
    /*****************************
         ...
         */
    R.compose(logResult, files.copyItem, files.getNextFile)();
...
function logCopyResult(item, today) {
  var successMessage = item.isError ? 'Error, ' + item.errMessage : 'Copied';
  log([
    successMessage,
    item.title,
