 ...
 */
Properties.prototype.load = function (files) {
    ...
    try {
        ...
        files.setFileList(JSON.parse(properties.fileList));
        files.setPageToken(JSON.parse(properties.pageToken));
    } catch (err) {
        throw err;
    }
 ...
 * for the application.
 */
Properties.prototype.save = function (files) {
    ...
    try {
        ...
        stringifiedProps.fileList = JSON.stringify(stringifyProps(files.getFileList()));
        stringifiedProps.pageToken = JSON.stringify(files.getPageToken());
    } catch (err) {
        throw err;
    }
