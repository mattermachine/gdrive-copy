 ...
 * permissions.js
 * 
 * Permissions object contains methods and properties for
 * interacting with the permissions on File objects
 * 
 ******************************************/
 ...
 */
Permissions.prototype.getPermissions = function (id) {
 ...
 */
// TODO: massive cleanup and refactoring needed
Permissions.prototype.copyPermissions = function (srcId, owners, destId) {
    var permissions, destPermissions, i, j;

    try {
        permissions = this.getPermissions(srcId).items;
    } catch (err) {
    ...
    try {
        destPermissions = this.getPermissions(destId).items;
    } catch (err) {
        log(null, [err.message, err.fileName, err.lineNumber]);
    }
    ...
    /**
     * Removing in conjunction with master branch, commit
     * https://github.com/ericyd/gdrive-copy/commit/981244524e790b0caeb320d51febe7deea45377e
     * 
     * Development branch is in completely different place than master,
     * but the reasons for removing from master apply here too.
     */
    ...
    //     var srcSS, destSS, srcProtectionsR, srcProtectionsS, srcProtection, destProtectionsR, destProtectionsS, destProtection, destSheet, editors, editorEmails, protect, h, k;
    //     try {
